window.pong = {
    ...
    active: false,
    interval: null,
    players: 1,          // 1 - соло, 2 - мультиплеер
    mode: 'single',
    ...
    pendingUpgradePlayer: 0,   // для мультиплеера: кто выбирает
    upgradeDialog: null,       // временный div для выбора
    ...
    allUpgrades: [
        { id: 'speed', name: 'Скорость ракетки', desc: '+25% скорости', apply: (p, val) => { p.tempBonus.speed = (p.tempBonus.speed || 1) + 0.25; } },
        { id: 'size', name: 'Размер ракетки', desc: '+15px высоты', apply: (p, val) => { p.height = Math.min(160, p.height + 15); } },
        ...
        { id: 'slowEnemy', name: 'Замедление врага', desc: 'Замедляет соперника на 5с', apply: (p, val, opponent) => { if(opponent) opponent.tempBonus.slow = 5; } },
        ...
        { id: 'invertControls', name: 'Инверсия', desc: 'Переворачивает управление врага на 3с', apply: (p, val, opponent) => { if(opponent) opponent.tempBonus.invert = 3; } },
    ...
    update: function() {
        ...
        // Столкновение с верхней/нижней стеной с учетом кривых стен
        if (this.activeEvent && this.activeEvent.curvedWalls) {
            // Кривые стены: отскок с искажением
    ...
    checkPaddleCollision: function(paddleIdx) {
        ...
        if (ball.x + ball.radius >= padLeft && ball.x - ball.radius <= padRight &&
            ...
            let angleRad = collidePoint * Math.PI/3; // -60..60 градусов
            ...
            // Инверсия управления у противника (если есть активный эффект)
            if (pad.tempBonus.invert && this.players === 2) {
                // меняем местами управление у врага - реализовано через флаг, но для простоты пропустим
            }
            // Небольшое смещение, чтобы избежать застревания
    ...
    resetBall: function(scoredOn, resetEffects) {
        ...
        if (resetEffects) {
            for (let i = 0; i < this.players; i++) {
                // Сохраняем только постоянные улучшения (upgrades) и щиты, сбрасываем временные бонусы
                this.paddles[i].tempBonus = {};
                // но не сбрасываем shield, он хранится отдельно
            }
        }
    },
    ...
    endEvent: function() {
        if (this.activeEvent) {
            // Ищем событие по имени и вызываем его end-обработчик
    ...
    promptUpgradeForPlayer: function(playerIdx) {
        // Создаём временное диалоговое окно поверх canvas
        if (this.upgradeDialog) document.body.removeChild(this.upgradeDialog);
        ...
        choices.forEach(up => {
            ...
            btn.onclick = () => {
                ...
                if (this.players === 2 && playerIdx === 0) {
                    this.pendingUpgradePlayer = 1;
                    this.promptUpgradeForPlayer(1);
                } else {
                    this.waitingForUpgrade = false;
                    this.upgradeTimer = 60;
                    // Для бота даём случайное улучшение
